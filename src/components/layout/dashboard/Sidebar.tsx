...
import Link from "next/link";
import { usePathname } from "next/navigation";
import { LayoutDashboard, Send, Inbox, Target, Gift, Settings, BaggageClaim } from "lucide-react";
        ...
        <nav className="flex-1 px-4 space-y-1">
          {navItems.map((item) => (
            <Link key={item.name} href={item.href} 
                  className={`flex items-center gap-3 px-4 py-3 rounded-lg ${pathname === item.href ? "bg-gray-100" : ""}`}>
              <item.icon className="w-5 h-5" /> {item.name}
            </Link>
          ))}
        </nav>
