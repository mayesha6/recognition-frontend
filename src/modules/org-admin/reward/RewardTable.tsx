        ...
        <div className=" overflow-hidden ">
            <table className="w-full text-left">
                <thead className="bg-gray-50/50 text-gray-400 text-xs uppercase tracking-wider border-b border-gray">
                    <tr>
                        {["Reward", "Description", "Points", "Stock", "Status", "Action"].map((h, index, array) => (
                            <th
                                key={h}
                ...
                <tbody className="divide-y divide-gray">
                    {rewards.map((item: any, i: number) => {
                        ...
                        const points = item.pointsRequired ?? item.points ?? 0;
                        const stock = item.quantity ?? item.stock ?? 0;
                        const isActive = item.status === "Active" || item.isActive === "ACTIVE" || item.isActive === true;

                        return (
                            <tr key={item._id || item.id || i} className="hover:bg-gray-50/50">
                                <td className="px-6 py-4 font-medium text-gray-900">{item.name}</td>
                                <td className="px-6 py-4 text-gray-600">{description}</td>
                                <td className="px-6 py-4 text-gray-600 font-semibold">{points}</td>
                                <td className="px-6 py-4 text-gray-600">{stock}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-600'}`}>
                                ...
                                <td className="px-6 py-4 text-gray-500 text-center">
                                    <div className="flex justify-center gap-3">
                                        ...
                                        <button onClick={() => onDelete(item._id || item.id)} className="text-gray-400 hover:text-red-600">
                                            <Trash2 size={18} />
                                        </button>
