...
export default function EditDepartmentModal({
  ...
  useEffect(() => {
    if (departmentData) {
      ...
      const matchedAdmin = admins.find((admin: any) => (admin._id || admin.id) === departmentData.adminId);
      setAdminId(departmentData.adminId || "");
      setSelectedAdmin(matchedAdmin || null);
      setIsOpenDropdown(false);
    }
  }, [departmentData, isOpen, admins]);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-[2px] flex items-center justify-center z-50 p-4">
      <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-xl">
        ...
        <button
          onClick={() => onSave({ ...departmentData, name, adminId })}
